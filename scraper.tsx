...
//const numeroAtendimento: NA = (process.env.NA_FORNECEDORES ?? '').slice(0, 22);
...
(async () => {
    ...
    for (const na of listaDeNa) {
        ...
        const numeroAtendimento: NA = (na ?? '').slice(0, 22);
        ...
        try {
            ...
            //Verifica se o painel suspenso de tratativas está aberto ou não
            ...
            for (const correspondencia of await page.getByText(regexBusca).all()) {
                ...
                for (const linha of await page.locator('app-tratativa table tbody tr:nth-child(n)').all()) {
                    ...
                    const carta = new TratativaCarta(numeroAtendimento, situacaoCarta, fornecedorCodigo.slice(22, 24), fornecedorCNPJ, ...args);
            ...
            console.log(`NA em ordem numero:${cont} - NA: ${na}`);
            cont++;


        } catch (error) {
            console.log(`Erro na NA ${na}`,error);
            lista.push(`Erro na NA${na}`);
            continue;
        }
    }
